...
pub fn property_state(
    ...
    .map(|(is_atomic, field)| {        
        ...
        if *is_atomic {        
            ...
            quote! { 
                #pascal_name(<#field_ty as frand_home_state::State>::Message) 
            }
        }        
    })
    .collect();
    ...
    .map(|(is_atomic, field)| {
        ...
        if *is_atomic {        
            quote! { 
                pub #field_name: frand_home_state::Node<#field_ty> 
            }
        } else {        
            quote! { 
                pub #field_name: <#field_ty as frand_home_state::State>::Property
            }
        }        
    })
    .collect();
        ...
        if *is_atomic {   
            ...
            quote! { 
                self.#field_name.apply_message(
                    <#field_ty as frand_home_state::State>::Message::State(
    ...
    .map(|(index, (is_atomic, field))| {
        ...
        if *is_atomic {        
            quote! { 
                #field_name: frand_home_state::Node::new(
                    frand_home_state::vec_pushed(&ids, #id), 
                    context,
                )
            }
        } else {        
            quote! { 
                #field_name: <#field_ty as frand_home_state::State>::Property::new(
                    frand_home_state::vec_pushed(&ids, #id), 
                    context,
                )
            }
    ...
    .map(|(index, (is_atomic, field))| {
        ...
        if *is_atomic {        
            quote! { 
                #field_name: frand_home_state::Node::new_default(
                    frand_home_state::vec_pushed(&ids, #id), 
                )
            }
        } else {        
            quote! { 
                #field_name: <#field_ty as frand_home_state::State>::Property::new_default(
                    frand_home_state::vec_pushed(&ids, #id), 
                )
            }
        }        
        ...
        if *is_atomic {   
            ...
            quote! {                 
                #id => Ok(Self::#pascal_name(
                    <#field_ty as frand_home_state::State>::Message::new(ids, index+1, value)
                ))
            }        
        }
    })
    ...
    quote! {
        ...
        impl frand_home_state::NodeValue for #state_name {}

        impl frand_home_state::State for #state_name {
            type Property = #state_property_name;
            type Message = #state_message_name;
        }

        impl frand_home_state::StateProperty for #state_property_name {
            ...
            fn apply_export<Msg: frand_home_state::StateMessage>(&mut self, state: Self::State) -> Msg {
            ...
            where
                ...
                Msg: frand_home_state::StateMessage,
                <Comp as yew::BaseComponent>::Message: From<Msg>,
            {
                Self { 
                    ids: frand_home_state::vec_pushed(&ids, 1),
        ...
        impl frand_home_state::StateMessage for #state_message_name {
